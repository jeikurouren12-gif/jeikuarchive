...
export async function onRequest(context) {
  ...
  const providedPassword = body.password;
  if (!providedPassword || providedPassword !== context.env.ADMIN_PASSWORD) {
    return jsonResponse({ error: 'Unauthorized' }, 401);
  }
  ...
  const owner = context.env.GITHUB_OWNER || context.env.GITHUB_REPO_OWNER || context.env.GITHUB_USER;
  const repo = context.env.GITHUB_REPO || context.env.GITHUB_REPOSITORY_NAME;
  const token = context.env.GITHUB_TOKEN;
  const branch = context.env.GITHUB_BRANCH || 'main';
  const filePath = context.env.FILE_PATH || 'data.json';
